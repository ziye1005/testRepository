package edu.njust.dal.impl;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONObject;
import edu.njust.dal.IKGraphRepository;
import edu.njust.entity.QAEntityItem;
import edu.njust.entity.dto.GraphPageRecord;
import edu.njust.query.GraphQuery;
...
public class KGraphRepository implements IKGraphRepository {
	...
	public void deleteKGdomain(String domain) {
		try {
			...
			neo4jUtil.executeCypherSql(rSql);
			String deleteNodeSql = String.format("MATCH (n:`%s`) delete n", domain);
			neo4jUtil.executeCypherSql(deleteNodeSql);
	...
	public void createdomain(String domain) {
		try {
			...
			neo4jUtil.executeCypherSql(cypherSql);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public HashMap<String, Object> createnode(String domain, QAEntityItem entity) {
		...
		try {
			if (entity.getUuid() != 0) {
				...
				String propertiesString = neo4jUtil.getFilterPropertiesJson(JSON.toJSONString(entity) + JSONObject.toJSONString(entity.getProperty()));
				String cypherSql = String.format("create (n:`%s`:`%s` %s) return n",
						domain,entity.getType(), propertiesString);
				graphNodeList = neo4jUtil.GetGraphNode(cypherSql);
			}
			if (graphNodeList.size() > 0) {
	...
	public List<HashMap<String, Object>> deletenode(String domain, long nodeid) {
		...
		try {
			...
			neo4jUtil.executeCypherSql(deleteRelationSql);
	...
	public void batchcreateGraph(String domain, List<Map<String, Object>> params) {
		try {
			if (params != null && params.size() > 0) {
				...
				neo4jUtil.executeCypherSql(nodeCypher);
				...
				neo4jUtil.executeCypherSql(relationShipCypher);
			}
	...
	public void batchInsertByCSV(String domain, String csvUrl, int status) {
		...
		loadRelCypher = " USING PERIODIC COMMIT 500 LOAD CSV FROM  '" + csvUrl + "' AS line " + " MATCH (m:`" + domain
				...
				+ "	SET r.name=line[2];";
		neo4jUtil.executeCypherSql(addIndexCypher);
		neo4jUtil.executeCypherSql(loadNodeCypher1);
		neo4jUtil.executeCypherSql(loadNodeCypher2);
		neo4jUtil.executeCypherSql(loadRelCypher);
	}
	...
	public void updateCorrdOfNode(String domain, String uuid, Double fx, Double fy) {
		...
		neo4jUtil.executeCypherSql(cypher);
	}
}
